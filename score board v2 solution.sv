...
bmu_sequence_item qPackets[$];// the wrong received packet but i take in order to take out the inputs to calculate the result in refmodel
bmu_sequence_item delayed_pcks_correct[$];
bmu_sequence_item compa[$];
bmu_sequence_item correctP;//from DUT 
bmu_sequence_item x;//from DUT 
//analysis port 
...
qPackets.push_back(seq);//receive it from monitor ===> synch issue so the scoreboard will receive the wrong one packet(result or even as ihave here result=b not old result) , so instead
//i will make q_delayed_received_packets[$] : 
//q_delayed_received_packets.push_pack(delayed_seq);
//this solution assume that the driver and monitor are working at the same edge !tahts why the delay will be 1 cycle ...
delayed_pcks_correct.push_back(seq);
endfunction
...
//wait(q_delayed_received_packets.size==2) oldpacket(wrong) + newpacket(correct)
//so once this condition has been satisfied so 100% qPackets.size!=0 so we will continue to calculate the result 
//based on the original packet ( from aPacket queue ) then we will compare it with the correct one 

wait(qPackets.size()!=0);
actualPacket=qPackets.pop_front();//wrong result packet
...
compa.push_back(refPacket);
//here the calculated result is ready
     wait (delayed_pcks_correct.size()==2) ;
   correctP= delayed_pcks_correct.pop_front();//the 2nd packet in the que is which contains the correct result so it is we will make the comparision with it (the inputs are not correct since ....)
/*actualPacket.result_ff=correctP.result_ff;
actualPacket.error=correctP.error;*/
x=compa.pop_front();
   if(if_equel(x,correctP)==1'b1)begin  //matched :)
      ...
      `uvm_info("pass", 
          $sformatf("A=%0d    B=%0d  Result=%0d   error=%b | RefA=%0d  RefB=%0d   RefResult=%0d   error=%b\n\n", 
                    correctP.a_in, correctP.b_in, correctP.result_ff,correctP.error, x.a_in,
                    x.b_in, x.result_ff, x.error),UVM_LOW)
   end
