...
class equality_sub_seq extends uvm_sequence #(bmu_sequence_item);
 ...
 //disable the Reset signal for the test of test cases 
   ...
   seq.ap.zba=1;
    ...
    `uvm_info(get_type_name(), ("Standard SUB with a_in = b_in (pos values )input case"), UVM_NONE) //faild
repeat(3) begin
 // a-b (a=b) (both are pos values) result=0 ,error=0
   seq.randomize() with {valid_in==1;csr_ren_in==1'b0;a_in==32'h7FFFFFFF; b_in==32'h7FFFFFFF;}; 
   start_item(seq); 
   finish_item(seq); 
end
   ...
   `uvm_info(get_type_name(), ("Standard SUB with a_in = b_in (neg values )input case"), UVM_NONE) //faild
repeat(3) begin
 // a-b (a=b) (both are neg values) result=0 ,error=0
   seq.randomize() with {valid_in==1;csr_ren_in==1'b0;a_in==32'hFFFFFFFF; b_in==32'hFFFFFFFF;}; 
   start_item(seq); 
   finish_item(seq); 
end
