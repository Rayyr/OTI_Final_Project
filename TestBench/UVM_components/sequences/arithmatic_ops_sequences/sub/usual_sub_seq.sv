...
class usual_sub_seq extends uvm_sequence #(bmu_sequence_item);
 ...
 //disable the Reset signal for the test of test cases 
   ...
   seq.ap.sub=1'b1;
    ...
    `uvm_info(get_type_name(), ("Standard SUB with a_in=100 , b_in=60 usual case without overflow/underflow"), UVM_NONE) //faild
repeat(3) begin
 //both inputs are + ( usual case without overflow )
   seq.randomize() with {valid_in==1;csr_ren_in==1'b0;a_in==32'h00000064; b_in==32'h0000003C;}; 
   start_item(seq); 
   finish_item(seq); 
end
    ...
    `uvm_info(get_type_name(), ("Standard SUB with a_in=-2330 , b_in=-400023 usual case without overflow/underflow"), UVM_NONE) //faild
repeat(3) begin
 //both inputs are - ( usual case without underflow )
   seq.randomize() with {valid_in==1;csr_ren_in==1'b0;a_in==32'hFFFFF6E6; b_in==32'hFFF9E569;}; 
   start_item(seq); 
   finish_item(seq); 
end
   ...
   `uvm_info(get_type_name(), ("Standard SUB with a_in=112035 , b_in=-22223044 usual case without overflow/underflow"), UVM_NONE) //faild
repeat(3) begin
 //neg and pos inputs ( usual case without underflow )
   seq.randomize() with {valid_in==1;csr_ren_in==1'b0;a_in==32'h0001B563; b_in==32'hFEAD084C;}; 
   start_item(seq); 
   finish_item(seq); 
