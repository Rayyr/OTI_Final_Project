...
class conflict_sub_data_path_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(conflict_sub_data_path_seq)
 


  //overriden new()
  function new(string name ="conflict_sub_data_path_seq");
    super.new(name);
  endfunction
  ...
  task body(); 
    ...
    //activate sra operation  
    seq.ap.sra=1'b1;
  


    `uvm_info(get_type_name(), ("Conflict SUB data path by activation other operation once at a time"), UVM_NONE) //faild
repeat(3) begin
    seq.randomize() with {csr_ren_in==1'b0;valid_in==1;};
