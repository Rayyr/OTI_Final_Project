...
class underflow_sub_seq extends uvm_sequence #(bmu_sequence_item);
 ...
 //disable the Reset signal for the test of test cases 
   ...
   `uvm_info(get_type_name(), ("Standard SUB with a_in=min 32-bit signed value , b_in=1 subtraction with underflow"), UVM_NONE) //faild
repeat(3) begin
 // underflow : min_input  -  1   result= 2_147_483_647(max)    error=1
   seq.randomize() with {valid_in==1;csr_ren_in==1'b0;a_in==32'h80000000; b_in==32'h00000001;}; 
   start_item(seq); 
   finish_item(seq); 
end
