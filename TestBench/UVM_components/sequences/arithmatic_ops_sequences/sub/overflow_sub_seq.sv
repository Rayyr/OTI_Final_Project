...
class overflow_sub_seq extends uvm_sequence #(bmu_sequence_item);
 ...
 // overflow : max_input  -  (-1)  result=-2_147_483_648 (min)   error=1
   seq.randomize() with { csr_ren_in==1'b0;a_in==32'h7FFFFFFF; b_in==32'hFFFFFFFF;}; 
   start_item(seq); 
   `uvm_info(get_type_name(), ("Standard SUB with a_in=max 32-bit signed value , b_in=-1 subtraction with overflow"), UVM_NONE) 
   finish_item(seq);
