...
class overflow_sub_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(overflow_sub_seq)
 


  //overriden new()
  function new(string name ="overflow_sub_seq");
    super.new(name);
  endfunction
 ...
 //disable the Reset signal for the test of test cases 
   ...
   seq.ap.sub=1'b1;
    ...
    `uvm_info(get_type_name(), ("Standard SUB with a_in=max 32-bit signed value , b_in=-1 subtraction with overflow"), UVM_NONE) //faild
repeat(3) begin
 // overflow : max_input  -  (-1)  result=-2_147_483_648 (min)   error=1
   seq.randomize() with {valid_in==1;csr_ren_in==1'b0;a_in==32'h7FFFFFFF; b_in==32'hFFFFFFFF;}; 
   start_item(seq); 
   finish_item(seq);
end
