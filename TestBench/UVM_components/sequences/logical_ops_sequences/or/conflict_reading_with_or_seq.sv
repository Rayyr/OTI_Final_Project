...
class conflict_reading_with_or_seq extends uvm_sequence#(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(conflict_reading_with_or_seq)
 


  //overriden new()
  function new(string name ="conflict_reading_with_or_seq");
    super.new(name);
  endfunction
  ...
  task body(); 
    ...
    `uvm_info(get_type_name(), ("Conflict OR data path by activation the read signal once at a time"), UVM_NONE) //passed
repeat(3) begin 
...
seq.ap.zbb=1;
    `uvm_info(get_type_name(), ("Conflict Inverted-OR data path by activation the read signal once at a time"), UVM_NONE) //passed
repeat(3) begin 
    seq.randomize() with {valid_in==1;csr_ren_in==1'b1;seq.rst_l==1'b1;};
    start_item(seq);//drive this transaction to the driver via the sequencer then to the DUT via the design_interface
    finish_item(seq);//notify that the process is finished ( sent sucessfully to the DUT )
