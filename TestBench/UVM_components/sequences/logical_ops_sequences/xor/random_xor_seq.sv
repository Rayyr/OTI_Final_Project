...
class random_xor_seq extends uvm_sequence#(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(random_xor_seq)
 


  //overriden new()
  function new(string name ="random_xor_seq");
    super.new(name);
  endfunction
  ...
  seq.ap.lxor=1'b1;
    ...
    `uvm_info(get_type_name(), ("Standard XOring with random inputs"), UVM_NONE) ;/// =============passed
...
seq.ap.zbb=1;
    `uvm_info(get_type_name(), ("Standard Inverted-XOring with random inputs"), UVM_NONE) ;/// =============passed
     //a=random  b=random    ( random inputs )
repeat(50) begin //50 random cases
     seq.randomize() with { valid_in==1'b1;csr_ren_in==1'b0; seq.rst_l==1'b1;};//i put it outside the loop due to have same randomized inputs
repeat(3) begin
