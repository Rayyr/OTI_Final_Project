...
class conflict_xor_data_path_seq extends uvm_sequence#(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(conflict_xor_data_path_seq)
 


  //overriden new()
  function new(string name ="conflict_xor_data_path_seq");
    super.new(name);
  endfunction
  ...
  task body(); 
    ...
    `uvm_info(get_type_name(), ("Conflict inverted xor data path by activation other operation once at a time"), UVM_NONE) //faild
repeat(3) begin
...
seq.ap.zbb=0;
    `uvm_info(get_type_name(), ("Conflict xor data path by activation other operation once at a time"), UVM_NONE) //faild
repeat(3) begin
    seq.randomize() with {valid_in==1;csr_ren_in==1'b0;seq.rst_l==1'b1;};
    start_item(seq);//drive this transaction to the driver via the sequencer then to the DUT via the design_interface
    finish_item(seq);//notify that the process is finished ( sent sucessfully to the DUT )
