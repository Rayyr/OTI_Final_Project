...
class random_max_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(random_max_seq)
 


  //overriden new()
  function new(string name ="random_max_seq");
    super.new(name);
  endfunction
  ...
  task body(); 
    ...
    `uvm_info(get_type_name(), ("Standard MAX random inputs"), UVM_NONE) //faild
    repeat(6) begin
