...
class usual_max_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(usual_max_seq)
 


  //overriden new()
  function new(string name ="usual_max_seq");
    super.new(name);
  endfunction
  ...
  task body(); 
   ...
   `uvm_info(get_type_name(), ("Standard MAX with both positive inputs"), UVM_NONE) //faild    it is min logic
repeat(3) begin
 //a_in=200  b_in=34  result=200     dut-result=34( both pos values )
   ...
   `uvm_info(get_type_name(), ("Standard MAX with both negative inputs"), UVM_NONE) //faild   it is min logic
repeat(3) begin
 //a_in=-200  b_in=-34  result=-34     dut-result=-200( both neg values )
   ...
   `uvm_info(get_type_name(), ("Standard MAX with positive and negative inputs"), UVM_NONE) //faild   it is min logic
repeat(3) begin
 //a_in=200  b_in=-34  result=200      dut-result=-34( pos & neg values )
