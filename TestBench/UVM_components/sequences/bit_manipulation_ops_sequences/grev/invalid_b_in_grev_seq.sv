...
class invalid_b_in_grev_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(invalid_b_in_grev_seq)
 


  //overriden new()
  function new(string name ="invalid_b_in_grev_seq");
    super.new(name);
  endfunction
 ...
 //disable the Reset signal for the test of test cases 
  ...
  // result=0  error=1
     `uvm_info(get_type_name(), ("Invalid b_in (b[4:0]!=24d) GREV case"), UVM_NONE) //passed
repeat(3) begin
