...
class random_grev_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(random_grev_seq)
 


  //overriden new()
  function new(string name ="random_grev_seq");
    super.new(name);
  endfunction
  ...
  task body(); 
   ...
   `uvm_info(get_type_name(), ("Standard GREV with random inputs)"), UVM_NONE) //faild   it reveres the complete 4 bytes 
repeat(5) begin
       seq.randomize() with {valid_in==1;csr_ren_in==1'b0;b_in[4:0]==5'b11000;rst_l==1;}; 
repeat(3) begin
    start_item(seq); 
   finish_item(seq);
end
