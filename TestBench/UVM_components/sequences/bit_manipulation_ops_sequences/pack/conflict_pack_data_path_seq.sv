...
class conflict_pack_data_path_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(conflict_pack_data_path_seq)
 


  //overriden new()
  function new(string name ="conflict_pack_data_path_seq");
    super.new(name);
  endfunction
  ...
  task body(); 
    ...
    `uvm_info(get_type_name(), ("Conflict PACK data path by activation other operation once at a time"), UVM_NONE) //faild
repeat(3) begin
