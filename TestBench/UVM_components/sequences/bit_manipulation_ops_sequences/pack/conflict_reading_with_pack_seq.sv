...
class conflict_reading_with_pack_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(conflict_reading_with_pack_seq)
 


  //overriden new()
  function new(string name ="conflict_reading_with_pack_seq");
    super.new(name);
  endfunction
  ...
  task body(); 
    ...
    `uvm_info(get_type_name(), ("Conflict PACK data path by activation the read signal once at a time"), UVM_NONE) //passed
repeat(3) begin
