...
class usual_pack_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(usual_pack_seq)
 


  //overriden new()
  function new(string name ="usual_pack_seq");
    super.new(name);
  endfunction
 ...
 //disable the Reset signal for the test of test cases 
   ...
   `uvm_info(get_type_name(), ("Standard PACK with 0 inputs"), UVM_NONE) //passed
repeat(3) begin
 //both inputs are zero
   ...
   `uvm_info(get_type_name(), ("Standard PACK with 1's-bits input"), UVM_NONE) //passed
repeat(3) begin
 //both inputs are 1's bits
   ...
   `uvm_info(get_type_name(), ("Standard PACK with a_in[31]=0  and b_in[15]=1 "), UVM_NONE) //faild
