...
class conflict_ctz_data_path_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(conflict_ctz_data_path_seq)
 


  //overriden new()
  function new(string name ="conflict_ctz_data_path_seq");
    super.new(name);
  endfunction
 ...
 seq.ap.rand_mode(0); 
   initialize_ap(seq.ap);
    seq.ap.ctz=1'b1;
    //activate cpop & sra operations once 
    seq.ap.cpop=1'b1;
    seq.ap.sra=1'b1;

    seq.randomize() with {csr_ren_in==1'b0; seq.rst_l==1'b1;seq.a_in==65;};
    start_item(seq);//drive this transaction to the driver via the sequencer then to the DUT via the design_interface
    `uvm_info(get_type_name(), ("Conflict CTZ data path by activation other operation once at a time"), UVM_NONE) 
