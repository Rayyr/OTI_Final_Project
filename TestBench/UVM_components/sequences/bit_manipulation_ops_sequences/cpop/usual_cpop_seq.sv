...
/*
 //a_in=0 result=0 error=0  ///passed
   seq.randomize() with { csr_ren_in==1'b0;a_in==32'h00000000;}; 
   start_item(seq); 
   `uvm_info(get_type_name(), ("Standard CPOP with 0 input"), UVM_NONE) 
   finish_item(seq);
   #10;*/


    //a_in=32'b11111...  result=32 ////falied when it is run with the oher seqs
 
   start_item(seq); 
     seq.randomize() with {b_in==45; valid_in==1'b1;csr_ren_in==1'b0;a_in==32'h00000000;rst_l==1'b1;}; 
       initialize_ap(seq.ap);
   seq.ap.cpop=1'b1;
   `uvm_info(get_type_name(), ("Standard CPOP with 1's input"), UVM_NONE) 
   
   finish_item(seq);
#10;

/*
  //a_in=101010(alternative input)  result=32/2     ///passed
   seq.randomize() with { valid_in==1'b1;csr_ren_in==1'b0;a_in==32'b10101010101010101010101010101010;rst_l==1'b1;}; 
   start_item(seq); 
