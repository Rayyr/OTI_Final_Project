...
class min_rotation_amount_ror_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(min_rotation_amount_ror_seq)
 


  //overriden new()
  function new(string name ="min_rotation_amount_ror_seq");
    super.new(name);
  endfunction
  ...
  task body(); 
    ...
    `uvm_info(get_type_name(), ("Min rotation amount case"), UVM_NONE) //passed
        seq.randomize() with {csr_ren_in==1'b0;b_in[4:0]==5'b00000;valid_in==1;};
repeat(3) begin
//a=any value , b=0decimal  result=a(same unchanged)
