...
class max_rotation_amount_ror_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(max_rotation_amount_ror_seq)
 


  //overriden new()
  function new(string name ="max_rotation_amount_ror_seq");
    super.new(name);
  endfunction
...
//a=any value , b=31decimal  
    seq.randomize() with {csr_ren_in==1'b0;b_in[4:0]==5'b11111;};
    start_item(seq);//drive this transaction to the driver via the sequencer then to the DUT via the design_interface
    `uvm_info(get_type_name(), ("Max rotation amount case"), UVM_NONE) 
