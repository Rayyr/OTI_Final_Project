...
class conflict_srl_data_path_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(conflict_srl_data_path_seq)
 


  //overriden new()
  function new(string name ="conflict_srl_data_path_seq");
    super.new(name);
  endfunction
...
//1st case : standard inverted XOR with random inputs a , b
   seq.ap.rand_mode(0);//disable the randomization for the op feilds once we randomize the seq , this globally will be turned off
   ...
   `uvm_info(get_type_name(), ("Conflict SRL data path case"), UVM_NONE) //faild
repeat(3) begin
   seq.randomize() with {valid_in==1;csr_ren_in==1'b0;};//these constraints locally for this line so this the difference of constraint_mode(0) vs inline constraint which is globally , locally respectivlly
