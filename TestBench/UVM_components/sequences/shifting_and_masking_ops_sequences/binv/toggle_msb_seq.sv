...
class toggle_msb_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(toggle_msb_seq)
  ...
  task body(); 
    ...
    `uvm_info(get_type_name(), ("Toggle MSB case with MSB=1"), UVM_NONE) 
        seq.randomize() with {csr_ren_in==1'b0;a_in[31]==1'b1; b_in[4:0]==5'b11111;valid_in==1;};//passed
repeat(3) begin
...
seq.randomize() with {csr_ren_in==1'b0; a_in[31]==1'b0 ; b_in[4:0]==5'b11111;valid_in==1;};//passed
repeat(3) begin
