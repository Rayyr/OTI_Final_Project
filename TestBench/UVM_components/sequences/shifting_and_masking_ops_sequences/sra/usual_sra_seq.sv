...
class usual_sra_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(usual_sra_seq)
 


  //overriden new()
  function new(string name ="usual_sra_seq");
    super.new(name);
  endfunction
 ...
 //disable the Reset signal for the test of test cases 
   ...
   start_item(seq);
   `uvm_info(get_type_name(), ("Standard SRA"), UVM_NONE) 
   finish_item(seq);
