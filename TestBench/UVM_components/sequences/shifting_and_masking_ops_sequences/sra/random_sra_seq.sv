...
class random_sra_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(random_sra_seq)
 


  //overriden new()
  function new(string name ="random_sra_seq");
    super.new(name);
  endfunction
...
//test all bs shift amount value 
   `uvm_info(get_type_name(), ("Standard SRA with random inputs"), UVM_NONE) //passed
 for(int c=0;c<=31;c++) begin
      seq.randomize() with {valid_in==1;csr_ren_in==1'b0;b_in==c;};
repeat(3) begin
   start_item(seq);
   finish_item(seq);
end
