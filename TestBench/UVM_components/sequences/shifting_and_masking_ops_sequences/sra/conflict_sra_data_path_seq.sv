...
class conflict_sra_data_path_seq extends uvm_sequence #(bmu_sequence_item);
  
  //register the class into uvm factory
  `uvm_object_utils(conflict_sra_data_path_seq)
 


  //overriden new()
  function new(string name ="conflict_sra_data_path_seq");
    super.new(name);
  endfunction
...
//1st case : standard inverted XOR with random inputs a , b
   ...
   `uvm_info(get_type_name(), ("Conflict SRA data path case"), UVM_NONE) //faild
repeat(3) begin
